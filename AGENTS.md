...
- 泳道式 DAG 编排界面：横向流动，多条并行支线，任务节点按状态着色
- 三层交互面：菜单栏（零成本 glance）→ 常驻悬浮窗（日常执行主界面）→ 主窗口（规划编排）
- 完成→推荐循环：完成任务后自动推荐下一个，零认知间隙
- Inbox 任务收集：快速输入，自动分配到对应支线
- 菜单栏指示器：常驻显示当前任务，快速完成/切换
- 多 project 源：支持手动创建，预留解耦接口供外部软件同步（Linear、GitHub 等）
